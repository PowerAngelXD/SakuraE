...
namespace sakuraE {
    ...
    public:
        ...
        static Constant make(Token tok) {
            switch (tok.type)
            {
            case TokenType::BOOL_CONST:
                return Constant(tok.content == "true"?true:false, tok.info);
            case TokenType::INT_N:
                return Constant(atoi(tok.content.c_str()), tok.info);
            case TokenType::FLOAT_N:
                return Constant(atof(tok.content.c_str()), tok.info);
            case TokenType::STRING:
                return Constant(tok.content, tok.info);
            case TokenType::CHAR:
                return Constant(tok.content.at(0), tok.info);
        ...
        template<typename T>
        static Constant make(T value, PositionInfo info) {
            return Value(value, info);
        }

        template<typename T>
        static Constant make(T value, IR::TypeToken tok, PositionInfo info) {
